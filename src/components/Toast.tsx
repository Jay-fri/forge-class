 ...
 * badge awards, submission confirmations, and other minor moments worth
 * a beat of feedback. Never use this for anything that needs a decision;
 * that's a blocking modal instead (see brand nav spec).
 */
export function Toast({ message, icon }: { message: string | null; icon?: ReactNode }) {
  return (
    <div className="pointer-events-none fixed inset-x-0 top-0 z-40 flex justify-center px-4 pt-4">
      <AnimatePresence>
        {message && (
          ...
          >
            {icon ?? <TrophyIcon className="text-accent" size={18} />}
            {message}
          </motion.div>
        )}
      </AnimatePresence>
